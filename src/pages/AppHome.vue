          ...
          <div class="message" v-for="(message, index) in selectedMessages.data" :key="index">
            ...
            <div class="message-body">
              <div v-if="message.role == ChatRole.user" class="detail-right">
                ...
                <div class="body-content content-user" v-html="message.content">
                </div>
              </div>
...
const selectedMessages = ref<any>(null);
const msgsEle = ref<any>(null);
...
function addedHeadMessage(headMessage:HeadMessage) {
...
function removeHeadMessage(index:number) {
...
function removedHeadMessage(headMessage:HeadMessage) {
  const headMessages = state.headMessage.data;
  stateStroge.set(state.headMessage.key, headMessages);
...
function resetLastMessage(headMessageIndex:number) {
...
function addMessage() {
  ...
  Chat(state.message.data[selectedHeadId].filter((m:ChatMessage)=>m.normal), state.userSearch, state.model.used(), cancelToken.token, res => {
...
function editHeadMessage(index:any) {
  state.tempHeadMsg = {};
  copy(state.headMessage.data[index], state.tempHeadMsg);
}

function changeHeadMsg(index:number) {
...
function selectMessages(index:number) {
  const headMessage = state.headMessage.data[index];
  const data = {
    headId: headMessage.id,
...
function freshResponse(index:number) {
  if(state.model.thinking){
    stopThinking();
    return;
  }
  ...
  const cancelToken = createCancelToken();
  state.model.thinking = true;
  Chat(data.filter((m:ChatMessage)=>m.normal), state.userSearch, state.model.used(), cancelToken.token, res => {
    const content = res.data;
    let i = 0;
    const timer = setInterval(() => {
      if (!state.model.thinking) {
